...
if __name__=="__main__":
    ...
    for i in range (0,100):
        ...
        print(f"Время исполнения: {exec_time}")
        timestats.append(float(exec_time.total_seconds()))
    timestats==[float(value) for value in sorted(timestats)]
    print(sum(timestats))
    n=len(timestats)
    m=max(timestats)
    mi=min(timestats)
    if n%2==0:
        median=(timestats[n//2-1]+timestats[n//2])/2
    else:
        median=timestats[n//2]
    avg=sum(timestats)/n
    print(f"Медианное время выполнения: {median}\nСреднее время выполнения :{avg}\nМаксимальное время выполнения: {m} \n Минимальное время выполнения {mi}")